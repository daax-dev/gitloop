...
// notifier; then drive a task to the pipeline's terminal step via real git
// pushes and check that the push → hook → notifier seam actually fires.
// Honors GITLOOP_CONFIG / <worker>/.gitloop/pipeline.json. Run it by hand to
...
import { firstStep, terminalStep } from '../src/core/config.js';
...
async function main(): Promise<void> {
  ...
  // Load the pipeline config the same way the MCP server and notifier do, so
  // the sandbox honors GITLOOP_CONFIG / <worker>/.gitloop/pipeline.json (arch-009)
  // instead of silently using the default.
  const config = loadPipelineConfigFromEnv(process.env, worker);
  const firstStepName = firstStep(config).name;
  const terminalName = terminalStep(config).name;
  // Generous bound scaled to the pipeline length so deeper custom pipelines
  // (or extra loop-back versions) still terminate, with headroom over the
  // ~steps*2 advances we expect for a single rejection round-trip.
  const iterationCap = config.steps.length * 4 + 10;
  ...
  let notifier = createNotifier({ port: PORT, repoPath: worker, config, log: () => {} });
  let boundPort = PORT;
  try {
    ...
    notifier = createNotifier({ port: 0, repoPath: worker, config, log: () => {} });
    ({ port: boundPort } = await notifier.start());
  }
  ...
  const deps = { git: g, remote: 'origin', config, workerId: 'sandbox' };
  let rejected = false;
  let reachedTerminal = false;
  let capExceeded = false;
  for (let i = 0; i < iterationCap && !reachedTerminal; i++) {
    if (i === iterationCap - 1) capExceeded = true;
    const { tasks } = await getPendingTasks(deps);
    const t = tasks.find((x) => x.taskId === 'TASK-1');
    if (!t) break;
    await claimTask(deps, { taskId: 'TASK-1' });
    // Reject once after the first step is reached. Config-driven, so any
    // pipeline with ≥2 steps demonstrates a real loop-back.
    if (!rejected && config.steps.length >= 2 && t.currentStep === firstStepName) {
    ...
    if (adv.step === terminalName && adv.advanced) reachedTerminal = true;
  }

  // Give fire-and-forget hook POSTs a moment to land.
  await new Promise((r) => setTimeout(r, 500));

  const res = await fetch(`http://127.0.0.1:${String(boundPort)}/events`);
  const body = (await res.json()) as { events: { id: number; tags: string[] }[] };
  const tags = await g.listTags();

  log('');
  log(
    `reached terminal (${terminalName}): ${String(reachedTerminal)}; rejection happened: ${String(rejected)}`,
  );
  ...
  if (!reachedTerminal) {
    findings.push(
      capExceeded
        ? `iteration cap (${String(iterationCap)}) exceeded before reaching terminal (${terminalName})`
        : `task did not reach terminal (${terminalName})`,
    );
  }
  if (!rejected && config.steps.length >= 2) findings.push('no rejection loop-back occurred');
  ...
  if (findings.length === 0) {
    log(
      `✅ no rough edges found: hook fired, events flowed, task reached ${terminalName} via a rejection.`,
    );
  } else {
    log('⚠️ rough edges:');
  ...
  log(`  GITLOOP_REPO=${worker} GITLOOP_NOTIFIER_PORT=${String(boundPort)} pnpm start:notifier`);
